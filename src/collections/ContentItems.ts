...
export const ContentItems: CollectionConfig = {
  slug: 'content-items',
  admin: { useAsTitle: 'title', defaultColumns: ['title', 'status', 'locale', 'updatedAt'] },
  ...
  fields: [
    ...
    { name: 'slug', type: 'text', required: true, localized: true, index: true },
    {
      ...
      options: ['article', 'campaign', 'landing_page', 'social_asset', 'video'].map((value) => ({ label: value, value })),
    },
    {
      name: 'status',
      type: 'select',
      required: true,
      defaultValue: 'draft',
      options: ['draft', 'in_review', 'changes_requested', 'approved', 'scheduled', 'published', 'archived'].map((value) => ({ label: value, value })),
      access: { update: ({ req }) => contentRoles.includes(String(req.user?.role)) },
    },
